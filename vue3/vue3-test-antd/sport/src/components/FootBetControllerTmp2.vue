<template>
  <div class="foot-bet-controller-tmp2">
    <!--底部注单-->
    <div class="bet-container">
      <!--一个注单 h5-->
      <div class="full-width"
           v-if="betValues.length === 1 && !showBetDetail"
           v-for="bet of betValues" :key="bet.signId">
        <van-swipe-cell right-width="100" ref="swipe-cell">
          <div class="bg-white pd-16-12" style="color: #545454;" @click.stop="_openSwipeCell">
            <div class="fs14">
              <span class="m-r8" style="color: #126E51;">{{ bet.title }}</span>
              <span>{{ bet.odds }}</span>
<!--              {{ (bet.group_code === 'incorrect_score' || bet.group_code === 'half_time_incorrect_score') ? '%' : '' }}-->
            </div>
            <div class="fs12 flex-between">
              {{ bet.market_name }}
              <span v-if="bet.isFBD == 1">
                {{$t('d_b')}} {{bet.limit_min}} - {{bet.limit_max}}
              </span>

              <span v-if="bet.isFBD == 1">{{$t('k_t')}} {{bet.left_amount || '0'}}</span>
            </div>
            <div class="dark-light">
              <span class="fs12">{{ bet.home_name }} V {{ bet.away_name }}</span>
            </div>
          </div>
          <template #right>
            <van-button square block :text="$t('del')" color="#a63838"
        ...
        <div class="flex-between" style="height:2.4rem;">
          <div class="full-height full-width">
            <van-field class="fs16 full-height only-one-input"
                       style="background-color: #333;"
                       :placeholder="statusAlias(bet).state ? $t('s_d_b_j') : statusAlias(bet).msg"
                       :disabled="!statusAlias(bet).state"
          ...
          <van-button block square
                      style="height:2.4rem;"
                      :color="!amount ? '#666' : betChanged.isChange ? '#58D7AF':'#126E51'"
                      @click="submitBet"
                      :loading="loading" :loading-text="$t('c_bets')">
                        <span class="fs16" :style="{color: betChanged.isChange ? '#333':'#fff'}">{{ betChanged.text }}</span>
            <span v-if="!betChanged.isChange" class="fs16 m-l10" style="color: #5FE8BD;">{{$utils.toCurrency(projectConfig.currency).siteCurrency}}&nbsp;{{ amount }}</span>
          </van-button>
        </div>
      </div>
    ...
    <van-popup v-model="showBetDetail"
      ...
      <div class="drawer-content-tmp2 over-hidden">
        <div class="bet-container-title over-hidden flex-between" style="padding-left:0.6rem;" @click.stop="showBetDetail = false">
          <div class="flex-middle-only">
            ...
            <div class="fs12" style="margin-left: 0.6rem;">
              ...
              <span class="block fs12" style="color:#126E51;" @click.stop="editing = !editing">{{ editing ? $t('over') : $t('edit') }}</span>
            </div>
          </div>
        ...
        <div class="bg-white flex-middle-only delete-all-content" v-if="editing">
