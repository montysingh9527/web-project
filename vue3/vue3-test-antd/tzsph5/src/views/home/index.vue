<template>
                    ...
                    <div class="content">
                        <div class="search">
                            <div style="display: flex; align-items: center">
                                ...
                                <van-image :src="banner_icon" width="85" style="margin-left: 25px" />
                            </div>
                            <div style="display: flex; align-items: center">
        ...
        <div class="swipers">
            <van-swipe :autoplay="3000" lazy-render>
                <van-swipe-item
                    ...
                    loop
                    show-indicators
                    @click="onClickBanner(item.hrefUrl)"
                >
                    <img v-lazyload="item.imgUrl" class="swiper_img" style="overflow: hidden; border-radius: 4px" />
                    <!-- <van-image :src="image" class="swiper_img" style="overflow: hidden; border-radius: 4px" /> -->
                </van-swipe-item>
            </van-swipe>
        </div>
        <div class="container" ref="containerRef">
            <div class="theme_list" v-if="state.result && state.result.length">
                <div class="list" v-for="item in state.result" :key="item.id">
                    <van-cell
                        is-link
                        @click="$router.push({ path: '/theme', query: { title: item.title, themeId: item.id } })"
                    >
                        <template #title>
                    ...
                    <van-row gutter="10">
                        <van-col
                            ...
                            v-for="e in item.ext"
                            :key="e.id"
                            @click="$router.push(`/play?id=${e.id}`)"
                        >
                            <ImgUrlBox :videoList="e" />
                            <div class="list_title title_line_2 van-multi-ellipsis--l2">{{ e.title }}</div>
                        </van-col>
                    </van-row>

                    <!-- <div class="list_item" v-for="e in item.ext" :key="e.id" @click="$router.push(`/play?id=${e.id}`)">
                        <ImgUrlBox :videoList="e" />
                        <div data-v-557fd7d6="" class="list_title title_line_1 van-ellipsis">
                            {{e.title}}
                        </div>
                    </div> -->
                    <img
                        ...
                        style="width: 100%"
                    />
                    <!-- <van-image src="http://img.cola9527taozi15.xyz/vod/meterial/20211207/VL0H0W37540038426527.png" /> -->
                </div>
            </div>
        </div>
    </div>

    <van-popup v-model:show="show" position="left" style="height: 100%; width: 290px" overlay closeable>
        <div class="drawer">
            ...
            <div class="drawer-body">
                <van-cell-group :border="false">
                    <van-cell :border="false" to="/vault">
                        <template #title>
                ...
                <van-cell :border="false" :url="kfUrl">
                    <template #title>
                        <div class="drawer-nav">
                            <i
                                style="
            ...
            <div class="drawer-footer">
                <a href="http://91down.gujinjituan.com/91porn.apk">
    ...
    >
        <div style="display: flex; align-items: center; padding: 5px 10px">
            <van-image :src="ico" width="40" />
            <div style="width: 52%; margin-left: 10px">
                <div style="font-size: 16px">下载APP，免费看视频</div>
                <div style="font-size: 12px">体验极致，玩游戏，赢取千万现金！！！</div>
            </div>
            <div style="display: flex; align-items: center">
                <a href="http://91down.gujinjituan.com/91porn.apk" style="display: flex; align-items: center">
                    <van-button
                        type="primary"
                        round
                        size="small"
                        style="color: rgb(51, 51, 51); width: 70px; margin-left: 10px"
                        >下载
                    </van-button>
                </a>
            </div>
...
import ico from '../../assets/img/ico.jpg'
...
.theme_list :deep(.van-cell) {
    padding: 0.26667rem 0;
}

.theme_list :deep(.van-cell__title) {
    overflow: hidden;
    white-space: nowrap;
