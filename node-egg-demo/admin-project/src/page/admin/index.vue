 ...
 * @FilePath: \web-project\node-egg-demo\admin-project\src\page\admin\index.vue
-->
<template>
  <div class="user-wrap">
    管理员账号
...
const init_table = async () => {
  ...
  const res = await api_admin.find_list(params);
...
const chang_create = async () => {
  ...
  const res = await api_admin.create_user(params);
...
const deleteData = async (rowData) => {
  await api_admin.delete_user({ id: rowData.id });
  init_table();
};
